...
bool if_end_with_all_wildcard(const char pattern[], int patPos);
int findSubLenAtStrPosWithPercent(const char str[], const char pattern[], int strPos, int patPos);
int findSubLenAtStrPosWith_all_4wildcards(const char str[], const char pattern[], int strPos, int patPos);
...
int findSubLenAtStrPosWithDot(const char str[], const char pattern[], int strPos = 0, int patPos = 0)
{
    ...
    {
        if (pattern[patPos + 1] == NULL_CHAR) // the entire pattern is matched,ending case
            return 1;
...
bool if_end_with_all_wildcard(const char pattern[], int patPos)
{
    ...
    {
        return if_end_with_all_wildcard(pattern,patPos+1);
    }
    return false;
}
...
int findSubLenAtStrPosWithQmark(const char str[], const char pattern[], int strPos = 0, int patPos = 0)
{
    if (pattern[patPos] != NULL_CHAR && str[strPos] == NULL_CHAR) // the substring is shorter than the pattern to match
    {
        if (if_end_with_all_wildcard(pattern,patPos))
        {
            return 0;
        }
        return NOT_FOUND;
...
int findSubLenAtStrPosWithPercent(const char str[], const char pattern[], int strPos = 0, int patPos = 0)
{
    ...
    if (pattern[patPos] == PERCENT )
    {
        ...
        int result = findSubLenAtStrPosWithPercent(str, pattern, strPos + 1, patPos); // check if the remaining part of the pattern 
                                                                                      // matches with that of the substring
        if (result != NOT_FOUND) // only return a match when the entire pattern is matched
            return result + 1;
...
int findSubLenAtStrPosWith_all_4wildcards(const char str[], const char pattern[], int strPos=0, int patPos=0)
{
    if (pattern[patPos] != NULL_CHAR && str[strPos] == NULL_CHAR) // the substring is shorter than the pattern to match
    {
        if (if_end_with_all_wildcard(pattern,patPos))
    ...
    if (pattern[patPos] == str[strPos])
    {
        if (pattern[patPos + 1] == NULL_CHAR) // the entire pattern is matched,ending case
            return 1;
        // otherwise, the match is only part way through
        int result = findSubLenAtStrPosWith_all_4wildcards(str, pattern, strPos + 1, patPos + 1); // check if the remaining part of the pattern 
        if (result != NOT_FOUND) // only return a match when the entire pattern is matched
            return 1 + result;    //gaihaole
    }
    ...
    else if (pattern[patPos] == QMARK)
    {
        ...
        if (result != NOT_FOUND) // only return a match when the entire pattern is matched
            return result + 1;
        result = findSubLenAtStrPosWith_all_4wildcards(str, pattern, strPos, patPos + 1);
        if (result != NOT_FOUND)
        {
            return result;
        }
    ...
    {   
        ...
        if (pattern[patPos] == NULL_CHAR)
            return 1;
        int result = findSubLenAtStrPosWith_all_4wildcards(str, pattern, strPos + 1, patPos); // check if the remaining part of the pattern 
                                                                                      // matches with that of the substring
        if (result != NOT_FOUND) // only return a match when the entire pattern is matched
            return result + 1;
        result = findSubLenAtStrPosWith_all_4wildcards(str, pattern, strPos, patPos + 1);
...
int matchSub(const char str[], const char pattern[], int &length, int start = 0)
{
    ...
    if (pattern[0]==CARET)
    {
        int testLength = findSubLenAtStrPosWith_all_4wildcards(str, pattern, start,1);
        if (testLength != NOT_FOUND) {
        length = testLength;
        return start;
        }
    ...
    {
        int testLength = findSubLenAtStrPosWith_all_4wildcards(str, pattern, start);
        if (testLength != NOT_FOUND) {
            length = testLength;
            return start;
        }
